...
async def _run() -> None:
    ...
    if settings.venue == "binance":
        ...
        mm_auto = (not mm_requested) or (
            len(mm_requested) == 1 and mm_requested[0] == "AUTO"
        )
        mm2_auto = (not mm2_requested) or (
            len(mm2_requested) == 1 and mm2_requested[0] == "AUTO"
        )
        if symbols_auto or sma_auto or mm_auto or mm2_auto:

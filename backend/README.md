...
Order-level reconcile (`engine/core/order_reconciliation.py`) compares venue `openOrders` to OMS working children on the same cadence as position reconcile. When `RECONCILE_SKIP_REST_WHEN_USER_DATA_FRESH=true` (default), periodic passes skip `openOrders` REST while `ORDER_TRADE_UPDATE` / `ACCOUNT_UPDATE` were recent; startup still runs one REST snapshot (`force_rest`). When `RECONCILE_CANCEL_ORPHANS=true` (default), venue-only orphans are cancelled and the `order_reconcile_mismatch` breaker is cleared once the books match. Children that look “local-only” because user-data lag left them **working** while they no longer appear on `openOrders` are refreshed via Binance `GET /fapi/v1/order` (`fetch_order_by_client_id`) so fills/cancels merge before any mismatch is counted; remaining drift still trips minor `order_reconcile_mismatch`.

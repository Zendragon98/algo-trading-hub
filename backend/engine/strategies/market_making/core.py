...
def signed_tape_pressure(
    ...
    *,
    ...
    tape_mode = (mode or "volume").strip().lower()
    if tape_mode == "count":
        total = feat.tape_bid_hit_count + feat.tape_ask_hit_count
        min_tr = max(1, int(min_tape_trades))
        if total < min_tr:
            return 0.0
...
def tape_pressure(feat: Features, settings: Settings) -> float:
    return signed_tape_pressure(
        feat,
        mode=settings.mm_tape_pressure_mode or "volume",
        min_tape_trades=int(settings.mm_min_tape_trades),
    )
...
def infer_price_tick_from_touch(
    *,
    best_bid: float | None,
    best_ask: float | None,
    mid: float,
    ...
    if bid is not None and ask is not None and ask > bid > 0:
        ...
        if book_spread <= tick * 1.05:
            return tick
        if mid > 0 and spread_bps is not None and float(spread_bps) > 0:
            implied = mid * float(spread_bps) / 10_000.0
            if implied > 0 and book_spread <= implied * 1.05:
                return book_spread
        return tick
    if mid > 0 and spread_bps is not None and float(spread_bps) > 0:
        return mid * float(spread_bps) / 10_000.0
    return tick
